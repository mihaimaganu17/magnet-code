...
from magnet_code.client.response import StreamEventType, TokenUsage
from magnet_code.context.manager import ContextManager
from magnet_code.prompts.system import get_compression_prompt


class ChatCompactor:
    def __init__(self, client: LLMClient):
        """Create a compactor which will call the llm `client` to compact the message up until this
        point"""
        self.client = client
    ...
    async def compress(self, context_manager: ContextManager) -> tuple[str | None, TokenUsage | None]:
        """Compress the message/context from the `context_manager` up until this point. Returns
        the summary string and the tokens used."""
